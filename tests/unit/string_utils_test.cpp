...
TEST_CASE("parse_filesize parses human-readable filesizes", "[utils][string][parse]") {
    SECTION("parses bytes") {
        REQUIRE(*parse_filesize("1024") == 1024);
        REQUIRE(*parse_filesize("1024B") == 1024);
    }

    SECTION("parses kilobytes") {
        REQUIRE(*parse_filesize("1KB") == 1024);
        REQUIRE(*parse_filesize("1KiB") == 1024);
    }

    SECTION("parses megabytes") {
        REQUIRE(*parse_filesize("1MB") == 1024 * 1024);
        REQUIRE(*parse_filesize("1MiB") == 1024 * 1024);
    }

    SECTION("parses gigabytes") {
        REQUIRE(*parse_filesize("1GB") == 1024LL * 1024 * 1024);
        REQUIRE(*parse_filesize("1GiB") == 1024LL * 1024 * 1024);
    }

    SECTION("parses decimal values") {
        REQUIRE(*parse_filesize("1.5GB") == static_cast<int64_t>(1.5 * 1024 * 1024 * 1024));
    }
...
TEST_CASE("format_bytes formats bytes in human-readable format", "[utils][string][format]") {
    REQUIRE(format_bytes(512) == "512.00 B");
    REQUIRE(format_bytes(1024) == "1.00 KiB");
    REQUIRE(format_bytes(1536) == "1.50 KiB");
    REQUIRE(format_bytes(1024 * 1024) == "1.00 MiB");
    REQUIRE(format_bytes(1024LL * 1024 * 1024) == "1.00 GiB");
}

TEST_CASE("parse_duration parses duration strings", "[utils][string][parse]") {
    SECTION("parses seconds") {
        REQUIRE(*parse_duration("123") == 123);
    }

    SECTION("parses MM:SS") {
        REQUIRE(*parse_duration("1:23") == 83);
        REQUIRE(*parse_duration("10:00") == 600);
    }

    SECTION("parses HH:MM:SS") {
        REQUIRE(*parse_duration("1:23:45") == 5025);
        REQUIRE(*parse_duration("0:05:30") == 330);
    }

    SECTION("returns nullopt for invalid input") {
        REQUIRE_FALSE(parse_duration("abc").has_value());
        REQUIRE_FALSE(parse_duration("1:2:3:4").has_value());
    }
}

TEST_CASE("format_seconds formats duration", "[utils][string][format]") {
    SECTION("formats MM:SS for under 1 hour") {
        REQUIRE(format_seconds(83) == "1:23");
        REQUIRE(format_seconds(600) == "10:00");
    }

    SECTION("formats HH:MM:SS for 1 hour+") {
        REQUIRE(format_seconds(5025) == "1:23:45");
        REQUIRE(format_seconds(3661) == "1:01:01");
    }
